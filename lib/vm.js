 ...
 */
function isolateSymbolSandbox(sandbox) {
	sandbox.Symbol = () => {
		return () => Symbol;
	};
	...
	sandbox.Symbol.iterator = Symbol.toStringTag;
	...
	sandbox.Symbol.for = (type) => {
		if (type !== 'nodejs.util.inspect.custom') {
...
class VM extends EventEmitter {
	...
	constructor(options = {}) {
		...
		if (sandbox) {
			this.setGlobals(isolateSymbolSandbox(sandbox));
		}
	}
